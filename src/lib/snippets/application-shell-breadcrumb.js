        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={item.label} className="flex items-center gap-1.5">
              {index > 0 && <IconChevronRight aria-hidden="true" size={14} className="text-slate-300 dark:text-slate-600" />}
  ...
  return (
    ...
    <div ref={shellRef} className="relative flex min-h-[520px] w-full overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-900 shadow-[0_14px_40px_rgb(15_23_42/0.08)] dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100">
      ...
      <div className="flex min-h-[520px] min-w-0 flex-1 flex-col">
        ...
        <header className="flex min-h-[58px] items-center justify-between border-b border-slate-200/70 bg-white px-4 dark:border-slate-800/70 dark:bg-slate-950 md:hidden"><button type="button" onClick={() => setMobileOpen(true)} aria-label="Open navigation" className="flex size-9 items-center justify-center rounded-xl text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"><IconMenu2 aria-hidden="true" size={20} /></button><span className="text-sm font-semibold tracking-tight text-slate-900 dark:text-slate-100">Nexo UI</span><span className="size-9" /></header>
