...
function SidebarContent({ activeItem, onClose, mobileOpen = false }) {
  return (
    <>
...
export default function ApplicationShellWithBreadcrumb({ children, activeItem = 'Dashboard', items = [] }) {
  ...
  return (
    <div className="relative flex min-h-[520px] w-full overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-900 shadow-[0_14px_40px_rgb(15_23_42/0.08)] dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100">
      ...
      <aside className="hidden min-h-[520px] w-[272px] shrink-0 flex-col border-r border-slate-200 bg-[#FAFAF8] px-4 py-6 text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 md:flex"><SidebarContent activeItem={activeItem} /></aside>
      ...
      <div className="flex min-h-[520px] min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 hidden min-h-[58px] items-center border-b border-slate-200/70 bg-white/85 px-6 backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-950/85 md:flex"><Breadcrumb items={items} /></header>
