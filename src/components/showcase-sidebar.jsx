...
import { ChevronRight, Command, Copy, Menu, X } from 'lucide-react';
...
export default function ShowcaseSidebar({ activeId }) {
  ...
  return (
    <>
      <button type="button" onClick={() => setMobileOpen(true)} aria-label="Open navigation" className="fixed top-4 left-4 z-40 rounded-lg p-2 text-slate-400 hover:bg-white/[0.06] hover:text-white lg:hidden"><Menu className="size-5" /></button>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[252px] flex-col border-r border-white/[0.08] bg-[#070a10] px-5 py-5 transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-2">
          ...
          <button type="button" onClick={closeNavigation} aria-label="Close navigation" className="rounded-lg p-2 text-slate-500 hover:bg-white/[0.06] hover:text-white lg:hidden"><X className="size-4" /></button>
        </div>
